...
@WebServlet("/ControladorReporteGerente2")
public class ControladorReporteGerente2 extends HttpServlet {

    private ModelCliente modelCliente = new ModelCliente();
    private ConversionesVariables cv = new ConversionesVariables();
        ...
        if (req.getSession().getAttribute("USER") == null) {
            ...
            try {
                ...
                if (monto != null) {
                    ...
                    List<ClienteSumaTransacciones> sumaTransacciones = modelCliente.clientesTransaccionesSumadas();
                    if (sumaTransacciones.isEmpty()) {
                        ...
                        Double comparacion = this.cv.stringToDouble(monto);
                        List<Cliente> clientes = new ArrayList<>();

                        for (ClienteSumaTransacciones temp : sumaTransacciones) {
                            if (temp.getCantidad() > comparacion) {
                                clientes.add(temp.getCliente());
                            }
                        }
                        if (clientes.isEmpty()) {
                            req.setAttribute("success", 3);
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        if (req.getSession().getAttribute("USER") == null) {
            ...
            try {
                ...
                InputStream input = new FileInputStream(req.getServletContext().getRealPath("/Resources/jasperReports/Gerente/ReporteGerente2.jrxml"));
